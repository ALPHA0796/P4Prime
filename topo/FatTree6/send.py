      ...
      for i in range(args.n): 
        while True:
            ...
            if now - pre_time > args.t:
                pre_time = now
                if args.m == 'tag':
                    tmp = pkt2/ IP(dst = args.ip, options = IPOption_TAG(pkt_id=i, prime_product=1))
                elif args.m == 'mri':
                    tmp = pkt2/ IP(dst = args.ip, options = IPOption_MRI(count=0, swtraces=[]))
                else:
                    tmp = pkt2/ IP(dst = args.ip, id = i)
                tmp = tmp/ UDP (sport=7777, dport=8888)
                # tmp = pkt1
                # tmp.show()
